...
use iced::mouse;
use iced::theme;
use iced::widget::{button, column, container, horizontal_rule, horizontal_space, row, text, Text};
use iced::window;
        ...
        let toasts = toasts
            ...
            .map(|(index, toast)| {
                container(column![
                    container(
                        row![
                            text(toast.title.as_str()).font(YY_FONT),
                            horizontal_space(Length::Fill),
                            button(Text::new("\u{e901}").font(ICON_FONT))
                    ...
                    horizontal_rule(1),
                    container(text(toast.body.as_str()).font(YY_FONT))
                        .width(Length::Fill)
                        .padding(5)
